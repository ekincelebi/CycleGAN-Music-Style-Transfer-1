...
from ops import (lrelu, batch_norm, conv2d, deconv2d, linear, conv2d_musegan, relu, instance_norm,
                 deconv2d_musegan)
import numpy as np
...
def generator_musegan_bar(input, output_dim, reuse=False, name='generator_bar'):
    ...
    with tf.variable_scope(name):
        ...
        h0 = relu(batch_norm(
                    deconv2d(h0, 1024, [1, 1], [1, 1], padding='VALID', name='g_h0_deconv'),
                    name='g_h0_deconv_bn'))
...
def generator_musegan_phase(input, output_dim, reuse=False, name='generator_phase'):
    with tf.variable_scope(name):
        ...
        h0 = tf.reshape(input, tf.stack([-1, 1, 1, input.get_shape()[1]]))
        h0 = relu(batch_norm(
                    deconv2d(h0, 1024, [2, 1], [2, 1], padding='VALID', name='g_h1_deconv'),
                    name='g_h1_deconv_bn'))
        h1 = relu(batch_norm(
                    deconv2d(h0, output_dim, [3, 1], [1, 1], padding='VALID', name='g_h2_deconv'),
                    name='g_h2_deconv_bn'))
...
def generator_resnet(image, options, reuse=False, name="generator"):
    ...
    with tf.variable_scope(name):
        ...
        def residule_block(x, dim, ks=3, s=1, name='res'):
            ...
            y = instance_norm(conv2d(y, dim, ks, s, padding='VALID',
        ...
        pred = tf.nn.sigmoid(conv2d(d3, options.output_c_dim, 7,
                             1, padding='VALID', name='g_pred_c'))
